...
generate_matrix<-function(X,Y, target_x,target_y, liczba_kerneli, s){
  ...
  #output: best_lambda,y_predict (predykcja target_y)
...
pomocnicza <-function(X,Y,typ_klasy=0, id_target=ncol(X[[1]])){
  if (typ_klasy==0){
    for (i in 1:length(X)){
      X[[i]][,id_target]<-as.numeric(as.character(X[[i]][,id_target]))
      Y[[i]][,id_target]<-as.numeric(as.character(Y[[i]][,id_target]))
    }}
  if (typ_klasy==1){
    for (i in 1:length(Y)){
      X[[i]][,id_target]<-as.numeric((X[[i]][,id_target]))
      Y[[i]][,id_target]<-as.numeric((Y[[i]][,id_target]))
    }}
  
  return(list(X,Y))
}

badanie_dokladnosci<- function(X,Y, id_target=ncol(X[[1]]), liczba_kerneli=1000,typ=0){
  #X - lista danych treningowych
  #Y -lista danych testowych
  #id_target : numer kolumny w ktorej sa klasy/targety
  ...
  Y<-p[[2]]
  #if(mode(X[,id_target])==)
  #response_train<-wektory_klas_liczby(X,Y,id_target)[[1]]
  response_train<-X[[1]][,id_target]
  
  #response_test<-wektory_klas_liczby(X,Y,id_target)[[2]]
  response_test<-Y[[1]][,id_target]
...
dokladnosc_<-function(X,Y, id_target=ncol(X[[1]]),liczba_ker=1000,typ_klasy=0){
...
dokladnosc_uciete<-function(X,Y,wartosc=0,opcja=0,liczba_kerneli=1000,typ_klasy= 0){
